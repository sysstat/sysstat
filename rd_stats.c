...
/*
 ***************************************************************************
 * Snapshot of /proc files contents, shared by the functions reading it.
 *
 * Let's consider the /proc/stat file.
...
static char *proc_file_buf[MULTI_R_NR_FILE] = {NULL};
static size_t proc_file_bufsize[MULTI_R_NR_FILE] = {0};
static size_t proc_file_buflen[MULTI_R_NR_FILE] = {0};
...
void invalidate_buffers(void)
{
	...
	for (i = 0; i < MULTI_R_NR_FILE; i++) {
		proc_file_buflen[i] = 0;
	}
}
...
/*
 ***************************************************************************
 * Open a stream on a /proc file contents: Take a snapshot if none exists
 * (and if file may have to be read several times during a collection cycle).
 ...
 *
 * IN:
 * @status_r	Set to MULTIPLE_R value to indicate that the /proc file
 *		may be read several times during a collection cycle
 *		(for instance, sadc will use this value with the /proc/stat
 * 		file because read_stat_cpu(), read_load_avg(),
 *		read_stat_pcsw() and get_online_cpu_list() will read it).
 *		Set to SINGLE_R value to indicate that the file will only
 *		be read once (for instance, iostat, mpstat and pidstat will
 *		use this value with the /proc/stat file because they will
 *		read it once).
 * @pf_index	Index corresponding to current /proc file.
 * @proc_file	Name of /proc file.
 *
 * RETURNS:
 * A stream on /proc/stat contents, or NULL if it couldn't be opened.
 ***************************************************************************
 */
FILE *open_proc_file_stream(int status_r, int pf_index, const char *proc_file)
{
	FILE *fp = NULL;

	if (status_r == MULTIPLE_R) {
		/* File contents will be read several times during a cycle */
		if (!proc_file_buflen[pf_index]) {
			/* No snapshot exists yet: Read file contents */
			refresh_proc_file(&proc_file_bufsize[pf_index],
					  &proc_file_buflen[pf_index],
					  &proc_file_buf[pf_index], proc_file);
		}
		if (proc_file_buflen[pf_index] > 0) {
			/* A snapshot exists: Parse it instead of the real file */
			fp = fmemopen(proc_file_buf[pf_index],
				      proc_file_buflen[pf_index], "r");
		}
	}
	if (fp == NULL) {
...
__nr_t read_stat_cpu(struct stats_cpu *st_cpu, __nr_t nr_alloc, int status_r)
{
	...
	if ((fp = open_proc_file_stream(status_r, MULTI_R_STAT_IDX, STAT)) == NULL) {
 ...
 * @status_r	Set to MULTIPLE_R value by sadc to indicate that the
 *		/proc/meminfo file may be read several times during a
 *		collection cycle.
 *		Set to SINGLE_R value by pidstat to indicate that the file
 ...
 ***************************************************************************
 */
__nr_t read_meminfo(struct stats_memory *st_memory, int status_r)
{
	FILE *fp;
	char line[128];

	if ((fp = open_proc_file_stream(status_r, MULTI_R_MEMINFO_IDX,
					MEMINFO)) == NULL)
		return 0;
...
__nr_t read_vmstat_swap(struct stats_swap *st_swap)
{
	...
	if ((fp = open_proc_file_stream(MULTIPLE_R, MULTI_R_VMSTAT_IDX, VMSTAT)) == NULL)
		return 0;
...
__nr_t read_diskstats_io(struct stats_io *st_io)
{
	...
	if ((fp = open_proc_file_stream(MULTIPLE_R, MULTI_R_DISKSTATS_IDX,
					DISKSTATS)) == NULL)
		return 0;
...
__nr_t read_net_dev(struct stats_net_dev *st_net_dev, __nr_t nr_alloc)
{
	...
	if ((fp = open_proc_file_stream(MULTIPLE_R, MULTI_R_NET_DEV_IDX,
					NET_DEV)) == NULL)
		return 0;
...
__nr_t read_net_ip(struct stats_net_ip *st_net_ip)
{
	...
	if ((fp = open_proc_file_stream(MULTIPLE_R, MULTI_R_NET_SNMP_IDX, NET_SNMP)) == NULL)
		return 0;
...
__nr_t read_net_ip6(struct stats_net_ip6 *st_net_ip6)
{
	...
	if ((fp = open_proc_file_stream(MULTIPLE_R, MULTI_R_NET_SNMP6_IDX, NET_SNMP6)) == NULL)
		return 0;
...
__nr_t read_meminfo_huge(struct stats_huge *st_huge)
{
	...
	if ((fp = open_proc_file_stream(MULTIPLE_R, MULTI_R_MEMINFO_IDX,
					MEMINFO)) == NULL)
		return 0;
